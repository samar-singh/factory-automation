                    ...
                    try:
                        ...
                        try:
                            ...
                            if not isinstance(extracted, dict):
                                logger.warning(f"AI returned non-dict response: {type(extracted)}")
                                extracted = {"items": [], "error": "Invalid response format"}
                        except json.JSONDecodeError as e:
                            logger.warning(f"Failed to parse AI response as JSON: {e}")
                            extracted = {"items": [], "error": "JSON parse error"}
                    ...
                    for i in range(len(results["documents"][0])):
                        ...
                        if min_quantity > 0:
                            ...
                            try:
                                ...
                                # If conversion fails, include the match anyway
                                logger.warning(f"Could not compare quantities: qty={qty}, min_quantity={min_quantity}")
                                matches.append(match)
                        else:
                            matches.append(match)
