...
class BaseAgent:
    ...
    async def run(
        ...
        try:
            ...
            # Run the agent
            result = await self.runner.run(self.agent, message, context=context)
    ...
    def as_tool(
        self, tool_name: Optional[str] = None, tool_description: Optional[str] = None
    ) -> Callable:
        ...
        description = (
            tool_description
            or f"Use the {self.name} agent to {self.agent.instructions[:100]}..."
        )
