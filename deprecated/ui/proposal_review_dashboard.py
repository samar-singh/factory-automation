...
class ProposalReviewDashboard:
        ...
        if proposal.analysis and proposal.analysis.inventory_matches:
            for match in proposal.analysis.inventory_matches[:10]:
                details["inventory_matches"].append({
                    ...
                    "confidence": match.get("confidence", 0) if match.get("confidence") is not None else 0
                })
    ...
    def format_proposal_display(self, details: Dict[str, Any]) -> str:
        ...
        if estimated_value is not None:
            value_str = f"${estimated_value:.2f} (estimated)"
        else:
            value_str = "$0.00"
...
"""
        ...
        for action in details['actions']:
            ...
            confidence = action.get('confidence', 0) if action.get('confidence') is not None else 0
            output += f"- Confidence: {confidence:.1%} | Risk: {action['risk']}\n"
        ...
        if details['inventory_matches']:
            ...
            for match in details['inventory_matches'][:5]:
                match_confidence = match.get('confidence', 0) if match.get('confidence') is not None else 0
                output += f"- **{match['tag_code']}**: {match['name']} (Confidence: {match_confidence:.1%})\n"
        ...
        if details['alternatives']:
            ...
            for alt in details['alternatives']:
                alt_confidence = alt.get('confidence', 0) if alt.get('confidence') is not None else 0
                output += f"- **{alt['action']}**: {alt['reason']} (Confidence: {alt_confidence:.1%})\n"
