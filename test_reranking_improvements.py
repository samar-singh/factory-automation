...
def format_results_comparison(
    query: str, v1_results: Dict, v2_results: Dict
) -> None:
    ...
    if v2_results['matches']:
        ...
        for i, match in enumerate(v2_results['matches'][:3], 1):
            print(f"{i}. {match['item_name']} ({match['item_code']})")
            print(f"   Confidence: {match['confidence_percentage']}% ({match['confidence_level']})")
            print(f"   Stock: {match['available_stock']}")
...
def run_comparison_tests():
    ...
    for query in test_queries:
        ...
        comparison = {
            ...
            'v1_confidence': v1_results['matches'][0]['confidence_score'] if v1_results['matches'] else 0,
            'v2_confidence': v2_results['matches'][0]['confidence_percentage'] if v2_results['matches'] else 0,
            'v1_time': v1_time,
            'v2_time': v2_time,
            'action_changed': v1_results['recommended_action'] != v2_results['recommended_action']
        }
    ...
    if confidence_improvements:
        ...
        print(f"Average Confidence Improvement: {avg_improvement:.1f} percentage points")
    
    # Performance
    avg_v1_time = sum(r['v1_time'] for r in comparison_results) / len(comparison_results)
    avg_v2_time = sum(r['v2_time'] for r in comparison_results) / len(comparison_results)
    ...
    with open('reranking_comparison_results.json', 'w') as f:
        json.dump({
            'summary': {
                ...
                'avg_confidence_improvement': avg_improvement if confidence_improvements else 0,
