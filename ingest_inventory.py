#!/usr/bin/env python3
"""Script to ingest Excel inventory files into ChromaDB with Stella embeddings"""
...
def main():
    """Main function to run inventory ingestion"""
    
    # Initialize ChromaDB client
    logger.info("Initializing ChromaDB client...")
    chroma_client = ChromaDBClient()
    
    # Initialize ingestion with Stella-400M embeddings
    logger.info("Initializing Excel ingestion with Stella-400M embeddings...")
    ...
    test_queries = [
        ...
        ("Myntra sustainable", "MYNTRA"),
        ("formal blue tag", None),
        ("available stock cotton", None)
    ]
    
    for query, brand_filter in test_queries:
        print(f"\nQuery: '{query}'" + (f" (Brand: {brand_filter})" if brand_filter else ""))
        ...
        if results:
            for i, result in enumerate(results, 1):
                ...
                print(f"   Code: {meta.get('trim_code', 'N/A')}, "
