...
def test_basic_reranking():
    ...
    reranked_results = reranker.rerank(
        query=query, documents=documents, top_k=3, return_scores=True
    )
    ...
    hybrid_reranker = HybridReranker(
        reranker=reranker, initial_weight=0.3, rerank_weight=0.7
    )

    hybrid_results = hybrid_reranker.rerank_with_hybrid_scoring(
        query=query, search_results=documents.copy(), top_k=3
    )
