...
def reset_and_ingest_enhanced():
    ...
    embedding_model = "stella-400m"  # From config.yaml - primary model (1024 dims, better accuracy)
    ...
    use_vision = False  # config.yaml: rag.use_vision_model = false (requires Together.ai API)
    ...
    print(f"  Items with images: {total_images} ({total_images/total_items*100:.1f}%)" if total_items > 0 else "")
    print(f"  Rows recovered: {total_recovered} ({total_recovered/total_items*100:.1f}%)" if total_items > 0 else "")
    ...
    for query in test_queries[:3]:
        ...
        if results['documents'][0]:
            ...
            print(f"   Top result: {meta.get('item_name', 'N/A')} ({meta.get('brand', 'N/A')})")
    ...
    if total_recovered > 0:
        improvements.append(f"🎉 Recovered {total_recovered} rows that would have been lost")
    if total_images > 0:
    ...
    print("\nNext Steps:")
    print("1. Install CLIP if not already: pip install git+https://github.com/openai/CLIP.git")
    print("2. Enable vision model (use_vision_model=True) to generate tag names from images")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Reset and ingest with enhanced ingestion")
    parser.add_argument("--collection", default="tag_inventory_stella_smart", help="Collection name (production default)")
