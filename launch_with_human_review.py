...
def create_combined_interface():
    ...
    review_interface = HumanReviewInterface(
        interaction_manager=human_manager, chromadb_client=chromadb_client
    )
    ...
    with gr.Blocks(title="Factory Automation System", theme=gr.themes.Soft()) as app:
            ...
            with gr.TabItem("📊 System Status"):
                ...
                with gr.Row():
                    ...
                    stop_monitoring_btn = gr.Button(
                        "⏹️ Stop Monitoring", variant="secondary"
                    )
                    gr.Button("📥 Export Metrics")
        ...
        start_monitoring_btn.click(
            fn=lambda: asyncio.run(start_monitoring()), outputs=[orchestrator_status]
        )

        stop_monitoring_btn.click(
            fn=lambda: asyncio.run(stop_monitoring()), outputs=[orchestrator_status]
        )
