...
class VisualSimilarityTester:
    ...
    def __init__(self):
        ...
        self.chroma_client = ChromaDBClient(
            collection_name="tag_inventory_stella_smart"
        )
    ...
    def test_image_to_image_search(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            sample_results = self.image_storage.collection.get(
                limit=5, include=["metadatas"]
            )
    ...
    def test_text_to_image_search(self) -> Tuple[bool, str]:
        ...
        for query in test_queries:
            ...
            try:
                ...
                logger.info(
                    f"  Found {len(results)} results, {images_found} with images"
                )
                ...
                for r in results[:3]:
                    ...
                    logger.info(
                        f"  - {meta.get('item_name', 'Unknown')} ({'WITH' if has_image else 'NO'} image)"
                    )

                results_summary.append(
                    f"{query}: {images_found}/{len(results)} with images"
                )
    ...
    def test_visual_diversity(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            return (
                True,
                f"Diversity score: {diversity_score:.3f} (higher is more diverse)",
            )
