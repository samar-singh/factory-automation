...
def main():
    ...
    for email_idx, email in enumerate(emails, 1):
        ...
        order_summary = {"matched": 0, "partial": 0, "not_found": 0, "total_value": 0}
            ...
            if matches:
                ...
                if best_match["score"] < 0.7 and len(matches) > 1:
                    ...
                    for alt in matches[1:3]:
                        print(
                            f"   - {alt['item']} (Stock: {alt['stock']}, Score: {alt['score']:.1%})"
                        )
            else:
        ...
        print(
            f"Fully Matched: {order_summary['matched']} ({order_summary['matched']/len(order_lines)*100:.0f}%)"
        )
        print(
            f"Partial Match: {order_summary['partial']} ({order_summary['partial']/len(order_lines)*100:.0f}%)"
        )
        print(
            f"Not Found: {order_summary['not_found']} ({order_summary['not_found']/len(order_lines)*100:.0f}%)"
        )
