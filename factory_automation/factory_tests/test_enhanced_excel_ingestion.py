...
def test_enhanced_ingestion():
    ...
    total_ingested = sum(
        r["items_ingested"] for r in results if r["status"] == "success"
    )
    total_images = sum(
        r.get("items_with_images", 0) for r in results if r["status"] == "success"
    )
    total_recovered = sum(
        r.get("rows_recovered", 0) for r in results if r["status"] == "success"
    )
...
def test_image_extraction_only():
    ...
    ingestion = IntelligentExcelIngestion(
        embedding_model="minilm", use_vision_model=False
    )
    ...
    if os.path.exists(test_file):
        ...
        if images:
            ...
            for row, img_data in list(images.items())[:5]:  # Show first 5
                print(
                    f"  - Row {row}: {img_data['filename']} ({len(img_data['base64'])} bytes base64)"
                )
        else:
            print(
                "No embedded images found - the Excel may use image indicators instead"
            )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--images-only", action="store_true", help="Test only image extraction"
    )
    parser.add_argument(
        "--with-vision",
        action="store_true",
        help="Enable vision model for tag name generation",
    )
