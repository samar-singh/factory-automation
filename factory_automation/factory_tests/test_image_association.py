...
def test_image_association_workflow():
    ...
    print(
        f"   - Estimated storage size: {image_stats.get('estimated_total_size_mb', 0):.2f} MB"
    )
    ...
    # Test queries
    test_queries = ["peter england shirt", "blue tag", "items with images"]
    ...
    for query in test_queries:
        ...
        for i, result in enumerate(results, 1):
            ...
            if result.get("image_data"):
                ...
                print(
                    f"     - CLIP embedding: {result['image_data'].get('has_clip_embedding', False)}"
                )
            else:
                print("     - ❌ No image data")
...
def test_clip_embeddings():
    ...
    try:
        ...
        ingestion = IntelligentExcelIngestion(
            embedding_model="all-MiniLM-L6-v2", use_clip_embeddings=True
        )
