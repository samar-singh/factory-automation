...
def test_all_files():
    ...
    excel_files = sorted(list(inventory_folder.glob("*.xlsx")) + list(inventory_folder.glob("*.xls")))
    ...
    print(f"   • Successful: {successful_files} ({successful_files/len(excel_files)*100:.1f}%)")
    ...
    for col_type, pattern_list in patterns.items():
        if pattern_list:
            unique_patterns = list(set(pattern_list))[:10]  # Show max 10 unique patterns
    ...
    print(f"   New intelligent system: {successful_files/len(excel_files)*100:.1f}% success rate")
    print(f"   Improvement: {(successful_files/len(excel_files)*100 - 25):.1f}% better!")
