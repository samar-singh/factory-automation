...
def test_chromadb():
    ...
    client.create_collection("test_inventory")
    ...
    results = client.query_collection(
        collection_name="test_inventory", query_texts=["blue tag"], n_results=1
    )
    print("✓ Query successful")
    print(
        f"  Found: {results['documents'][0] if results['documents'] else 'No results'}"
    )
