...
async def test_system():
    ...
    print(
        f"✓ Together API Key: {'✓ Set' if settings.together_api_key else '✗ Missing'}"
    )

    # 4. Test agent creation (without running them)
    print("\n4. Testing Agent Creation...")
    try:
        from factory_automation.factory_agents.email_monitor_agent import (
            EmailMonitorAgent,
        )
        from factory_automation.factory_agents.inventory_matcher_agent import (
            InventoryMatcherAgent,
        )
        from factory_automation.factory_agents.order_interpreter_agent import (
            OrderInterpreterAgent,
        )
        ...
        InventoryMatcherAgent(chromadb_client=client)
