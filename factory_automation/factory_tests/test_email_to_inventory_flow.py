...
def generate_order_summary():
    ...
    for order in test_orders:
        ...
        if result['matches']:
            ...
            print(f"Match: {match['item_name']} (Confidence: {match['confidence_score']:.1%})")
            print(f"Stock: {match['available_stock']} units")
    ...
    print(f"Auto-processable: {summary['auto_processable']} ({summary['auto_processable']/summary['total_orders']*100:.0f}%)")
    print(f"Needs Review: {summary['needs_review']} ({summary['needs_review']/summary['total_orders']*100:.0f}%)")
    print(f"No Matches: {summary['no_matches']} ({summary['no_matches']/summary['total_orders']*100:.0f}%)")
    print(f"Insufficient Stock: {summary['insufficient_stock']} ({summary['insufficient_stock']/summary['total_orders']*100:.0f}%)")
