...
class GmailAgentEnhanced(BaseAgent):
    ...
    def _process_attachment(
        ...
        try:
            ...
            # Image files
            elif "image" in mime_type or filename.endswith(
                (".png", ".jpg", ".jpeg", ".gif")
            ):
    ...
    def _process_excel(self, data: bytes, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            relevant_cols = [
                ...
                if any(oc in col.lower() for oc in order_columns)
            ]
    ...
    def _process_pdf(self, data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            extracted = {"type": "pdf", "pages": len(pdf_reader.pages), "text": []}

            # Extract text from each page
            for page_num, page in enumerate(pdf_reader.pages):
                text = page.extract_text()
                if text.strip():
                    extracted["text"].append({"page": page_num + 1, "content": text})
    ...
    def extract_complete_order(self, email_content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if email_content.get("attachment_data"):
            ...
            for att_data in email_content["attachment_data"]:
                if att_data["type"] == "excel" and att_data.get("content", {}).get(
                    "data"
                ):
                    # Add Excel items
        ...
        if order.get("attachment_items"):
            ...
            for att_item in order["attachment_items"]:
                if att_item["type"] == "excel":
                    ...
                    item_desc = " ".join(
                        [f"{k}: {v}" for k, v in att_item["data"].items()]
                    )
    ...
    def process_order_email(self, msg_id: str) -> Dict[str, Any]:
        ...
        order["summary"] = {
            ...
            "has_excel": any(
                a["type"] == "excel" for a in message_data.get("attachment_data", [])
            ),
            "has_pdf": any(
                a["type"] == "pdf" for a in message_data.get("attachment_data", [])
            ),
            "has_images": any(
                a["type"] == "image" for a in message_data.get("attachment_data", [])
            ),
        }
