...
class GmailProductionAgent:
    ...
    def __init__(self, credentials_path: str, attachment_dir: Optional[str] = None):
        ...
        if attachment_dir:
            ...
            self.attachment_dir = Path(tempfile.gettempdir()) / "factory_automation_attachments"
            self.attachment_dir.mkdir(parents=True, exist_ok=True)
        
        logger.info(f"Gmail agent initialized. Attachments will be saved to: {self.attachment_dir}")
    ...
    def _download_attachments(self, message_id: str, payload: Dict) -> List[Dict[str, Any]]:
        ...
        message_dir = self.attachment_dir / f"msg_{message_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
