...
class InventoryRAGAgent(BaseAgent):
    ...
    def extract_brand_from_text(self, text: str) -> Optional[str]:
        ...
        known_brands = [
            'ALLEN SOLLY', 'MYNTRA', 'PETER ENGLAND', 'AMAZON',
            'LIFE STYLE', 'NETPLAY', 'VH DLS', 'WOTNOT'
        ]
    ...
    def _generate_status_message(self, status: str, matches: List[Dict], quantity: int) -> str:
        """Generate human-readable status message"""
        
        if status == 'matched_with_stock':
            match = matches[0]
            return (f"Found matching item: {match['item_name']} "
            ...
            return (f"Found {len(matches)} potential matches. "
                   f"Best match: {matches[0]['item_name']} "
                   f"with {matches[0]['confidence_score']:.0%} confidence. "
                   "Human review recommended.")
        
        elif status == 'low_confidence':
            return (f"Found {len(matches)} possible matches but confidence is low "
    ...
    def find_alternatives(self, item_code: str, min_stock: int = 0) -> List[Dict[str, Any]]:
    ...
    def run(self, task: str) -> str:
        ...
        if result['recommended_action'] == 'insufficient_stock' and result['matches']:
            ...
            alternatives = self.find_alternatives(
                result['matches'][0]['item_code'],
                min_stock=result['quantity_needed']
            )
            if alternatives:
