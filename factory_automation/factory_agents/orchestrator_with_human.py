...
class OrchestratorWithHuman(AgenticOrchestratorV3):
    ...
    def __init__(self, chromadb_client: ChromaDBClient):
        ...
        super().__init__(chromadb_client)
        
        # Initialize human interaction manager
        self.human_manager = HumanInteractionManager()
        
        # Register notification handler
        self.human_manager.register_notification_handler(self._handle_review_notification)
    ...
    def _add_human_tools(self):
        ...
        async def create_human_review(
            ...
            try:
                ...
                logger.debug(f"email_data type: {type(email_data)}, value: {email_data}")
                logger.debug(f"search_results type: {type(search_results)}, value: {search_results}")
                logger.debug(f"extracted_items type: {type(extracted_items)}, value: {extracted_items}")
            ...
            review = await self.human_manager.create_review_request(
                ...
                search_results=results_list if isinstance(results_list, list) else [results_list],
                confidence_score=confidence_score,
                extracted_items=items_list if isinstance(items_list, list) else [items_list]
            )
        ...
        async def get_pending_reviews(priority: Optional[str] = None) -> str:
            ...
            reviews = await self.human_manager.get_pending_reviews(priority_filter=priority_enum)
        ...
        async def process_review_decision(request_id: str) -> str:
            ...
            if review.status.value not in ["approved", "rejected", "alternative_suggested"]:
    ...
    def _get_enhanced_instructions(self) -> str:
        ...
        base_instructions = super()._get_agent_instructions()
        
        human_instructions = """

Human Interaction Guidelines:
1. When search confidence is between 60-80%, create a human review request
2. Monitor pending reviews and process decisions when available
3. For urgent emails, set high priority on review requests
4. Track review metrics to improve automation thresholds
5. Alternative suggestions from humans should be sent to customers

Review Workflow:
1. If confidence 60-80%: create_human_review → monitor status → process decision
2. Check for completed reviews periodically
3. Process approved reviews immediately
4. Handle rejected reviews with customer communication
5. Send alternative suggestions when provided by reviewers

Available Human Interaction Tools:
- create_human_review: Create manual review request
...
- process_review_decision: Handle completed review decision"""
        ...
        return base_instructions + human_instructions
        ...
        while self.is_monitoring:
            try:
                ...
                for review in reviews:
                    ...
                    if review.status.value in ["approved", "rejected", "alternative_suggested"]:
