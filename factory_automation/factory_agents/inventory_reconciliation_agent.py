...
class InventoryReconciliationAgent:
    ...
    def __init__(self, chromadb_client: ChromaDBClient):
        ...
        self.excel_source_path = Path(settings.get('inventory_excel_path', './inventory'))
        self.reconciliation_threshold = settings.get('reconciliation_threshold', 10)  # Units
    ...
    async def perform_reconciliation(
        ...
        if auto_fix:
            ...
            execution_results = {"status": "review_required", "actions_pending": actions}
        ...
        report = self._generate_report(
            reconciliation_id, excel_data, postgresql_data, 
            chromadb_data, discrepancies, actions, execution_results
        )
        
        return report
    
    async def _load_excel_inventory(self, file_path: Optional[str] = None) -> Dict[str, Any]:
            ...
            with get_db() as session:
                ...
                for row in results:
                    inventory_data[row.product_code] = {
                        ...
                        'quantity_total': row.quantity_available + row.quantity_reserved,
        ...
        try:
            ...
            if results and results['ids']:
                for i, item_id in enumerate(results['ids']):
                    ...
                    product_code = metadata.get('product_code', metadata.get('tag_code', ''))
                    if product_code:
                        inventory_data[product_code] = {
                            'source': 'chromadb',
                            'id': item_id,
                            'quantity': int(metadata.get('stock', metadata.get('quantity', 0))),
    ...
    def _find_discrepancies(
        ...
        all_codes = set(excel_data.keys()) | set(postgresql_data.keys()) | set(chromadb_data.keys())
        ...
        for product_code in all_codes:
            ...
            if not excel_item:
                ...
                discrepancies.append({
                    ...
                    'exists_in': [DataSource.POSTGRESQL if pg_item else None,
            ...
            if excel_item and pg_item:
                ...
                pg_qty = pg_item.get('quantity_total', pg_item.get('quantity_available', 0))
                ...
                if abs(excel_qty - pg_qty) > self.reconciliation_threshold:
                    discrepancies.append({
                        ...
                        'severity': 'high' if abs(excel_qty - pg_qty) > 100 else 'medium',
            ...
            with get_db() as session:
                session.execute(
                    ...
                    """),
                    {
                        ...
                        'status': 'completed' if not results.get('failed') else 'partial'
                    }
                )
                session.commit()
    ...
    def _generate_recommendations(
        ...
        missing_in_pg = [d for d in discrepancies 
                        if d.get('source_missing') == DataSource.POSTGRESQL]
        ...
        chroma_issues = [d for d in discrepancies 
        ...
        large_discrepancies = [d for d in discrepancies 
