...
class ProductionOrchestrator(AgenticOrchestratorV3):
    ...
    async def start_email_monitoring(self):
        ...
        logger.info(
            f"Starting Gmail monitoring (polling every {self.polling_interval} seconds)"
        )
                    ...
                    for email_data in emails:
                        try:
                            ...
                            if result.get("success"):
                                logger.info(
                                    f"Successfully processed: {email_data['message_id']}"
                                )
                                ...
                                if "recommended_action" in result:
                                    ...
                                    if action == "human_review":
                                        await self._notify_human_review_needed(
                                            email_data
                                        )
                                ...
                                logger.error(
                                    f"Failed to process: {email_data['message_id']}"
                                )
                                logger.error(f"Error: {result.get('error')}")

                        except Exception as e:
                            logger.error(
                                f"Error processing email {email_data['message_id']}: {e}"
                            )
    ...
    async def _notify_human_review_needed(self, email_data: Dict[str, Any]):
        ...
        logger.info(
            f"NOTIFICATION: Human review needed for order from {email_data['from']}"
        )
...
async def main():
    ...
    try:
        ...
        while True:
            ...
            print(
                f"Attachment storage: {stats['file_count']} files, {stats['total_size_mb']} MB"
            )
