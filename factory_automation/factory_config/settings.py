...
class Settings(BaseSettings):
    ...
    database_password: Optional[str] = Field("password", description="Database password")
        ...
        if config_path.exists():
            with open(config_path, 'r') as f:
                ...
                if yaml_config:
                    ...
                    kwargs.setdefault('app_env', app_config.get('environment', 'development'))
                    kwargs.setdefault('app_port', app_config.get('port', 8001))
                    kwargs.setdefault('gradio_port', app_config.get('gradio_port', 7860))
                    kwargs.setdefault('log_level', app_config.get('log_level', 'INFO'))
                    
                    # Orchestrator settings
                    orch_config = yaml_config.get('orchestrator', {})
                    kwargs.setdefault('use_ai_orchestrator', orch_config.get('use_ai_version', False))
                    kwargs.setdefault('enable_comparison_logging', orch_config.get('enable_comparison_logging', False))
                    
                    # Email settings
                    email_config = yaml_config.get('email', {})
                    kwargs.setdefault('email_poll_interval', email_config.get('poll_interval', 300))
                    kwargs.setdefault('email_folder', email_config.get('folder', 'INBOX'))
                    
                    # Gmail settings
                    gmail_config = yaml_config.get('gmail', {})
                    kwargs.setdefault('gmail_redirect_uri', gmail_config.get('redirect_uri', 'http://localhost:8080'))
                    kwargs.setdefault('gmail_token_file', gmail_config.get('token_file', 'token.json'))
                    
                    # ChromaDB settings
                    chroma_config = yaml_config.get('chromadb', {})
                    kwargs.setdefault('chroma_persist_directory', chroma_config.get('persist_directory', './chroma_data'))
                    kwargs.setdefault('chroma_host', chroma_config.get('host', 'localhost'))
                    kwargs.setdefault('chroma_port', chroma_config.get('port', 8000))
                    
                    # Security settings
                    security_config = yaml_config.get('security', {})
                    kwargs.setdefault('jwt_algorithm', security_config.get('jwt_algorithm', 'HS256'))
                    kwargs.setdefault('access_token_expire_minutes', security_config.get('access_token_expire_minutes', 1440))
                    
                    # Monitoring settings
                    monitoring_config = yaml_config.get('monitoring', {})
                    kwargs.setdefault('comparison_log_dir', monitoring_config.get('comparison_log_dir', './orchestrator_comparison_logs'))
    ...
    def get_chromadb_collections(self) -> Dict[str, str]:
        ...
        return self.config.get('chromadb', {}).get('collection_names', {
    ...
    def get_gmail_scopes(self) -> List[str]:
        ...
        return self.config.get('gmail', {}).get('scopes', [
