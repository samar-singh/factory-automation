...
{{
    ...
    "recommendations": []
        ...
        response = self.client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are an expert at analyzing Excel data structures and identifying schemas."},
    ...
    def parse_excel_intelligently(self, file_path: str) -> Tuple[pd.DataFrame, Dict[str, str]]:
    ...
    def _apply_schema_mappings(self, df: pd.DataFrame, mappings: Dict[str, str]) -> pd.DataFrame:
    ...
    def extract_products_with_ai(self, df: pd.DataFrame, brand: str) -> List[Dict[str, Any]]:
    ...
    "search_keywords": ["keyword1", "keyword2", ...]
            ...
            try:
                response = self.client.chat.completions.create(
                    model="gpt-3.5-turbo",
                    messages=[
                        {"role": "system", "content": "Extract structured product data from text."},
                        {"role": "user", "content": prompt}
                    ],
                    temperature=0.1
                )
...
class SelfImprovingExcelIngestion:
    ...
    def _assess_extraction_quality(self, products: List[Dict]) -> Dict[str, float]:
        ...
        metrics = {
            ...
            'has_keywords': sum(1 for p in products if p.get('search_keywords')) / total
        }
