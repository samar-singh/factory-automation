...
class IntelligentExcelIngestion:
    ...
    def __init__(
        ...
        self.embedding_model = embedding_model or model_config.get(
            "text_embedding", "all-MiniLM-L6-v2"
        )
        self.use_vision_model = (
            use_vision_model
            if use_vision_model is not None
            else rag_config.get("use_vision_model", False)
        )
        self.use_clip_embeddings = (
            use_clip_embeddings
            if use_clip_embeddings is not None
            else rag_config.get("use_clip_embeddings", True)
        )
    ...
    def identify_column_type(self, column_name: str, sample_values: List) -> str:
        ...
        if sample_values:
            ...
            if numeric_count > len(sample_values[:10]) * 0.7:
                ...
                has_decimals = any(
                    "." in str(v) for v in sample_values[:10] if pd.notna(v)
                )
    ...
    def extract_embedded_images(self, excel_file: str) -> Dict[int, Dict]:
        ...
        try:
            with zipfile.ZipFile(excel_file, "r") as zip_ref:
                ...
                if media_files:
                    logger.info(
                        f"Found {len(media_files)} embedded images in {excel_file}"
                    )
                    ...
                    try:
                        ...
                        if hasattr(ws, "_images") and ws._images:
                            for idx, img in enumerate(ws._images):
                                if hasattr(img, "anchor") and hasattr(
                                    img.anchor, "_from"
                                ):
    ...
    def generate_tag_name_from_image(self, image_data: Dict, brand: str = "") -> str:
        ...
        try:
            ...
            analysis = self.vision_engine.analyze_tag_image(
                image_base64=image_data["base64"], custom_prompt=prompt
            )
        ...
        for idx, row in df.iterrows():
            ...
            if has_identifier:
                ...
                if current_item is not None and size_rows:
                    combined = self._combine_size_rows(
                        current_item, size_rows, df.columns
                    )
    ...
    def _combine_size_rows(
        self, main_row: pd.Series, size_rows: List[pd.Series], columns
    ) -> Dict:
        """Combine main item row with its size variations"""
        combined = (
            main_row.to_dict() if hasattr(main_row, "to_dict") else dict(main_row)
        )
        ...
        qty_cols = [
            c for c in columns if any(q in str(c).lower() for q in ["qty", "quantity"])
        ]
        ...
        for qty_col in qty_cols:
            if pd.notna(combined.get(qty_col)):
                quantities.append(
                    float(combined[qty_col])
                    if str(combined[qty_col]).replace(".", "").isdigit()
                    else 0
                )
    ...
    def ingest_excel_file(self, file_path: str, batch_size: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                if (
                    not row_data.get("item_name")
                    or str(row_data.get("item_name")).strip() == ""
                ):
                    ...
                    if row_data.get("item_code"):
                        ...
                        generated_name = self.generate_tag_name_from_image(
                            image_data, brand
                        )
                        if generated_name:
                            ...
                            rows_recovered += 1
                            logger.info(
                                f"Generated tag name for row {idx}: {generated_name}"
                            )
                ...
                if image_data:
                    ...
                    if image_stored:
                        ...
                        items_with_images += 1
                        logger.debug(
                            f"Stored full image for row {idx} with ID: {image_id}"
                        )
                    else:
                ...
                # Generate meaningful tag name for easier identification
                from ..factory_agents.update_tag_names import (
                    generate_tag_name_for_ingestion,
                )

                tag_name = generate_tag_name_for_ingestion(
                    brand=brand,
                    item_name=row_data.get("item_name", ""),
                    item_code=row_data.get("item_code", ""),
                    customer_name=None,  # Not available during Excel ingestion
            ...
            dedup_manager = DeduplicationManager(self.chroma_client)
            ...
            for i in range(0, len(documents), batch_size):
                ...
                for j, (doc, meta, id_, emb) in enumerate(
                    zip(batch_docs, batch_meta, batch_ids, embeddings)
                ):
                    is_duplicate, existing_id = dedup_manager.check_before_insert(
                        self.chroma_client.collection.name, meta, doc, emb.tolist()
                    )
                    ...
                    if not is_duplicate:
                        ...
                        logger.debug(
                            f"Skipped duplicate: {meta.get('item_name')} (exists as {existing_id})"
                        )

                # Add non-duplicates to ChromaDB
                if filtered_docs:
                    ...
                    total_added += len(filtered_docs)

                logger.info(
                    f"Added batch {i//batch_size + 1}, added: {len(filtered_docs)}, skipped: {duplicates_skipped}, total: {total_added}/{len(documents)}"
                )
    ...
    def ingest_folder(self, folder_path: str) -> List[Dict[str, Any]]:
        ...
        results = []
        excel_files = list(Path(folder_path).glob("*.xlsx")) + list(
            Path(folder_path).glob("*.xls")
        )
